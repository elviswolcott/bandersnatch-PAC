...
const getChoices = () => {
  return Array.from(document.getElementsByClassName('BranchingInteractiveScene--choice-selection'))
    .map(nodeToOption);
}
...
const choose = choices => {
  ...
  arr.forEach(choice => {
    ...
    Array.from(document.getElementsByClassName('BranchingInteractiveScene--choice-selection'))
      .forEach(el => {
...
const ignoreReal = e => {
  if(e.isTrusted) {
    ...
    fireInfo("I know that you were planning to make your own selection, and I'm afraid that's something I cannot allow to happen.");
    e.preventDefault();
    e.stopPropagation();
    log("Prevented event");
  }
...
const onPlayerChanged = (mutationList) => {
  mutationList.forEach(change => {
    ...
    if(relevantNodes.length > 0) {
      ...
      var time = document.getElementsByClassName("BranchingInteractiveScene--horizontal-timer-container")[0];
      var options = {
        childList: false,
        attributes: true,
        subtree: false,
