<template>
  <div class="remote">
    <div class="countdown-container">
      <div
        class="countdown"
        :style="{ width: `${Math.min((timerPos / timerEnd) * 100, 100)}%` }"
      ></div>
    </div>
    <div class="progress">
      <div
    ...
    <div
      ...
      :key="option.cmp"
      @click="choose(option.cmp)"
      :class="{ ignore: option.ignore, selected: option.selected }"
    >
      <p v-if="option.type === 'text'">{{ option.data }}</p>
      <div
        class="option-image"
        v-if="option.type === 'image'"
        :style="{ backgroundImage: option.data.replace('webp', 'png') }"
      ></div>
    </div>
  </div>
</template>
